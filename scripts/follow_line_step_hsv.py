...
class LineFollower(object):
    ...
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber("/rosbots/camera1/image_raw",Image,self.camera_callback)
    ...
    def camera_callback(self,data):
        ...
        rows_to_watch = 100
        top_trunc = 1*height / 2 - 10 #get 3/4 of the height from the top section of the image
        ...
        if angular_z !=0:
            last_nz_ang = angular_z
        twist_object = Twist()
        if ((cx == width/2) and (cy == rows_to_watch/2)):
            rospy.loginfo("LINEAR VALUE SENT===>"+str(0))
            twist_object.linear.x = 0.0
            if last_nz_ang > 0:
                rospy.loginfo("ANGULAR VALUE SENT===>"+str(2))
                twist_object.angular.z = 2
            else:
                rospy.loginfo("ANGULAR VALUE SENT===>"+str(-2))
                twist_object.angular.z = -2
        else:
            rospy.loginfo("LINEAR VALUE SENT===>"+str(2))
            rospy.loginfo("ANGULAR VALUE SENT===>"+str(angular_z))
            twist_object.linear.x = 2.0
